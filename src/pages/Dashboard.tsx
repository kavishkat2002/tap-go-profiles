...
import { Smartphone, Plus, Eye, QrCode, LogOut } from "lucide-react";
import ProfileCard from "@/components/ProfileCard";
import QRCodeDisplay from "@/components/QRCodeDisplay";
import { DEMO_PROFILES } from "@/lib/types";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog";

const totalViews = DEMO_PROFILES.reduce((s, p) => s + (p.views ?? 0), 0);

export default function Dashboard() {
  const baseUrl = window.location.origin;

  const pathFor = (p: typeof DEMO_PROFILES[0]) =>
      ...
      <header className="border-b bg-card">
        <div className="container flex h-16 items-center justify-between">
          ...
          <Button variant="ghost" size="sm" asChild>
            <Link to="/"><LogOut className="h-4 w-4 mr-2" /> Logout</Link>
          </Button>
        </div>
      </header>
          ...
          <Card className="card-elevated">
            <CardContent className="pt-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-display font-bold">{DEMO_PROFILES.length}</p>
